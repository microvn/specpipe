---
description: Read-only root-cause investigation — OPTIONAL branch before /ap-fix. Produces an investigation report with potential root cause hypotheses, evidence, blast radius — no code changes. Use when bug is complex, ambiguous, production-critical, or user explicitly wants to diagnose before fixing (outage, data corruption, regression, unclear stack trace, "it was working yesterday"). Skip for trivial bugs — go straight to /ap-fix. Writes docs/investigate/<slug>-YYYY-MM-DD.md and hands off to /ap-fix.
...
and actionable next steps for whoever will fix it (human or `/ap-fix`).
...
→ To fix: run `/ap-fix <paste root cause summary>`
```
...
After producing the report, write it to `docs/investigate/<slug>-$(date +%Y-%m-%d).md` so `/ap-fix` can auto-detect it and skip redundant discovery.
...
```
⚠️ Ready to fix — run `/ap-fix docs/investigate/<slug>-<date>.md`

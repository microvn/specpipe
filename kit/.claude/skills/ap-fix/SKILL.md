...
description: |
  ...
  data corruption), start with /ap-investigate first, then hand the report to /ap-fix.
...
0. **Investigation handoff check.** If `$ARGUMENTS` references a file under `docs/investigate/`, read it first — it contains pre-built root cause hypothesis, blast radius, and recommended actions from `/ap-investigate`. Skip redundant discovery; jump to Phase 1 using its findings. If no such file and the bug is complex/ambiguous/production-critical → suggest the user run `/ap-investigate "<bug>"` first; otherwise proceed.
...
Draw a coverage diagram for the buggy function using the same format as ap-build:

...
mf-scaffold MAY offer this as a default the user confirms; the Bootstrap Brief always wins,
...
Widely adopted, composable, typed end-to-end. The scaffold's ONE example module wires a single slice through all of them — one query (React Query) → input validated by Zod → render — so every mf-build story copies an established pattern instead of improvising (this is what keeps cross-feature seams consistent).

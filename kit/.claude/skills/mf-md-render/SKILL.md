...
description: |
  ...
  long-form markdown that is not a /mf-plan spec (investigation reports,
  ...
  writes a long markdown artefact finishes (e.g. /mf-investigate,
  /mf-explore, /retro).

  For /mf-plan spec files (structured stories + acceptance scenarios),
  use [[mf-spec-render]] instead — it understands S-NNN / AS-NNN /
...
**Refuse politely** if the file looks like a /mf-plan spec (has `### S-\d{3}:` headings) — tell the user to run `/mf-spec-render` instead. Quick check: `grep -q '^### S-[0-9]\{3\}:' <file>`.
...
2. Reject if it looks like a /mf-plan spec (see above).
...
Walk the markdown and decide, for each chunk, the best component (see `components.md`). This is the core difference from `mf-spec-render`: there is no fixed schema. Use judgment.
...
- **Source is a /mf-plan spec** → STOP, tell user to run `/mf-spec-render`.
...
- ❌ Rendering a spec file — defer to [[mf-spec-render]].

---

## Relationship with other skills

- [[mf-spec-render]] — sibling for /mf-plan specs (story cards, AS dl/dt, P-badges). This skill is the generic fallback.
- `/mf-investigate`, `/mf-explore`, `/retro` produce long markdown artefacts; suggest this skill at the end so the user can render an HTML view.

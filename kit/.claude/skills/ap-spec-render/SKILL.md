...
description: |
  ...
  "preview spec", or after /ap-plan finishes writing or updating a spec.

  This skill is user-invoked, not auto-called. /ap-plan only writes the spec
  ...
  README) — those go to the generic sibling skill [[ap-md-render]] instead.
...
- ❌ Rendering when `<feature>.md` does not exist — STOP and tell the user to run `/ap-plan` first.

---

## Relationship with /ap-plan

This skill is decoupled from `/ap-plan`. `/ap-plan` writes the spec markdown and ends — it never calls this skill automatically. At the end of Phase 4 (new spec) and Mode C C5 (spec update), `/ap-plan` displays a 1–2 line hint telling the user to run `/ap-spec-render <feature>` if they want the HTML view.
...
- After `/ap-plan` finishes — generate or refresh the HTML view.

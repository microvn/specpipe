...
description: |
  ...
  long-form markdown that is not a /ap-plan spec (investigation reports,
  ...
  writes a long markdown artefact finishes (e.g. /ap-investigate,
  /ap-explore, /retro).

  For /ap-plan spec files (structured stories + acceptance scenarios),
  use [[ap-spec-render]] instead — it understands S-NNN / AS-NNN /
...
**Refuse politely** if the file looks like a /ap-plan spec (has `### S-\d{3}:` headings) — tell the user to run `/ap-spec-render` instead. Quick check: `grep -q '^### S-[0-9]\{3\}:' <file>`.
...
2. Reject if it looks like a /ap-plan spec (see above).
...
Walk the markdown and decide, for each chunk, the best component (see `components.md`). This is the core difference from `ap-spec-render`: there is no fixed schema. Use judgment.
...
- **Source is a /ap-plan spec** → STOP, tell user to run `/ap-spec-render`.
...
- ❌ Rendering a spec file — defer to [[ap-spec-render]].

---

## Relationship with other skills

- [[ap-spec-render]] — sibling for /ap-plan specs (story cards, AS dl/dt, P-badges). This skill is the generic fallback.
- `/ap-investigate`, `/ap-explore`, `/retro` produce long markdown artefacts; suggest this skill at the end so the user can render an HTML view.

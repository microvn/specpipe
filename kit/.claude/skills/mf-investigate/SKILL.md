...
and actionable next steps for whoever will fix it (human or `/mf-fix`).
...
→ To fix: run `/mf-fix <paste root cause summary>`
```
...
After producing the report, write it to `docs/investigate/<slug>-$(date +%Y-%m-%d).md` so `/mf-fix` can auto-detect it and skip redundant discovery.
...
```
⚠️ Ready to fix — run `/mf-fix docs/investigate/<slug>-<date>.md`

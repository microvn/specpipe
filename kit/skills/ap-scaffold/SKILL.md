...
description: |
  ...
  "create the project skeleton", or after /ap-explore confirms a greenfield build.
  ...
  and the user wants to start building — ap-build assumes a runnable harness exists; this is
  what creates it.
  Hands off to /ap-plan (first feature spec) → /ap-build. Skip if a runnable project already
  exists — go straight to /ap-plan.
...
1. **Greenfield check (idempotent).** Look for an existing runnable project in the target dir: any of `package.json`, `pyproject.toml` / `requirements.txt`, `Cargo.toml`, `go.mod`, `build.gradle`, `*.sln`, `Package.swift`, `Gemfile`, or a non-empty `src/`.
   - Found AND it builds → **wrong tool.** Stop: "A project already exists here — run `/ap-plan` to spec the next feature, then `/ap-build`." Do not re-scaffold.
...
2. **Resolve the bootstrap brief.** This skill needs: **app-type**, **stack** (with one-line rationale per major choice), preferred **scaffold command**, and the **smoke-test command**.
   - From `/ap-explore` greenfield branch → read its bootstrap brief (in `docs/explore/<feature>.md` or `$ARGUMENTS`).
...
2. **Stack — research current versions, propose, then confirm (never silently default).**
   ...
   **Stack-decision axes** (resolve each that applies to the app-type):
   ...
   | Test runner | The framework's blessed runner (this resolves `TEST_CMD` for ap-build). |
   ...
   **Confirmation question** — ap-scaffold is self-contained; do not depend on reading another skill for the format:
   ```json
...
3. **prove it runs** — demonstrate liveness the way THIS app-type requires (a server "boots" is not a library "imports" — don't conflate them). There MUST be at least one real, passing test so `TEST_CMD` resolves — this is exactly what unblocks `/ap-build`'s Phase 0b foundation gate.
...
Fill the templates in `references/`. At bootstrap the docs describe the **skeleton honestly** — not a hallucinated full system. They thicken as `/ap-build` adds stories.
...
1. **Secret/safety scan** (reuse `/ap-commit` discipline): no hardcoded secret; `.gitignore` excludes `.env`, `*.pem`, `*.key`, build dirs, `node_modules`/vendor; `.env.example` present and committed.
...
3. **Handoff summary:**
   ...
   - Next: "Run `/ap-plan <feature>` for the first feature spec, then `/ap-build`. `/ap-build`'s Phase 0b will re-verify the harness."
...
6. **Scaffolding is not behaviour.** This skill writes no acceptance scenarios and runs no TDD loop — that's `/ap-plan` + `/ap-build`. It only makes the harness those need exist.

...
description: |
  ...
  describes a new feature in vague or ambiguous terms — running /ap-plan with
  unclear requirements produces unclear specs.
  Skip if the user already has detailed acceptance criteria written down.
  Hands off to /ap-plan when discovery is complete — or, for a brand-new project
  with no codebase yet, also decides app-type + stack and hands off to /ap-scaffold first.
...
There is no codebase yet, so before boundaries/data, decide WHAT KIND of app this is and WHICH stack — this pre-fills the **Bootstrap Brief** that `/ap-scaffold` consumes to stand up a runnable skeleton. By Phase 1–2 you know the problem and the flow; that's enough to pick a stack. Keep it light — propose sensible defaults and let the client confirm, same posture as Phase 2.5. The canonical stack-decision matrix lives in `/ap-scaffold`; here you only pin the answers.
...
Compile before handing off to ap-plan:
...
**Bootstrap Brief:** _(GREENFIELD PROJECT only — consumed by `/ap-scaffold`; omit entirely for a feature on an existing codebase)_
...
- **Smoke contract:** [what "runs" means for this app-type — see /ap-scaffold Phase 3]
...
ap-plan reads this sketch and routes by tag:
...
ASCII stays in explore as provenance. If sketch carries no E/N/X legend, ap-plan treats all components as `[N]` and emits a Clarification flagging the assumption.

If neither sketches nor a prototype URL exist for a UI-bearing feature → ap-plan emits `GAP-NNN (status: open)` about UI structure.]
...
{
  "questions": [{
    "question": "Is this summary complete enough to hand off to /ap-plan?",
    "header": "Handoff",
    "multiSelect": false,
    "options": [
      {"label": "Yes, save & hand off", "description": "Write to docs/explore/; greenfield → /ap-scaffold, else → /ap-plan"},
...
If A → write to `docs/explore/<feature-slug>.md`. **If GREENFIELD PROJECT:** next step is `/ap-scaffold` (it reads the Bootstrap Brief to stand up the runnable skeleton), THEN `/ap-plan` for the first feature spec. Otherwise hand straight to `/ap-plan` — it auto-detects this file and skips redundant discovery when run on the same feature.
...
- [ ] Handoff summary is structured enough for /ap-plan to use directly without re-asking basic discovery questions
- [ ] If GREENFIELD PROJECT: app-type + stack decided with rationale; Bootstrap Brief filled for /ap-scaffold

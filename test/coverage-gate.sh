#!/usr/bin/env bash
# test/coverage-gate.sh — Regression tests for mf-build's Spec Coverage Gate (Phase 3.5)
#
# The gate is the one DETERMINISTIC part of mf-build: it counts coverage with a
...
SKILL="$(cd "$(dirname "$0")/.." && pwd)/kit/.claude/skills/mf-build/SKILL.md"
PASSED=0
FAILED=0
...
# ─── The gate, exactly as mf-build Phase 3.5 defines it ────────────────────────
